...
class EditAlarmFragment : Fragment() {
    ...
    private fun subscribeUi(binding: FragmentEditAlarmBinding) {
        viewModel.baseAlarm.observe(viewLifecycleOwner) { alarm ->
            binding.alarm = alarm
            val days = alarm!!.days
            val selectedDays = mutableListOf<MaterialDayPicker.Weekday>()
        ...
        binding.save.setOnClickListener {
            if (viewModel.insertOrUpdateAlarm(getPickDays(binding))) {
                viewModel.addAlarmNotification(requireContext())
                exit()
            } else {
                if (viewModel.workout == null) {
                    Toast.makeText(context, "Add workout to alarm.", Toast.LENGTH_LONG).show()
                } else {
                    Toast.makeText(context, "Alarm with such name already exists.", Toast.LENGTH_LONG).show()
                }
            }
        }
    ...
    private fun getPickDays(binding: FragmentEditAlarmBinding): Map<Converters.DayOfWeek, Boolean> {
        val days = mutableMapOf<Converters.DayOfWeek, Boolean>()
